    ...
    class Preload
    {
        ...
        public void PreloadThings()
        {
            ...
            foreach (string DataG in this.GameListFiles)
            {
                ...
                if (!ConfUCS.IsConsoleMode)
                    UI.SplashScreen.SS.Dispatcher.BeginInvoke(
                                                              (Action)
                                                              delegate()
                                                              {
                                                                  UI.SplashScreen.SS.PB_Loader.Value = this.Inc
                                                                                                       + (50
                                                                                                          * this
                                                                                                              .localvalue);
                                                              });
            ...
            if (!ConfUCS.IsConsoleMode)
                UI.SplashScreen.SS.Dispatcher.BeginInvoke(
                                                          ...
                                                          delegate()
                                                          {
                                                              UI.SplashScreen.SS.label_txt.Content =
            ...
            if (this.MissingNotReqFiles.Count != 0)
            {
                ...
                MessageBox.Show(IsMoreThanOne + this.MonoLine, "Missing Gamefile", MessageBoxButton.OK,
                ...
                if (!System.IO.File.Exists(System.IO.Directory.GetCurrentDirectory() + @"\" + DataG))
                {
                    MessageBox.Show(
                                    string.Format(
                                                  ...
                                                  DataG, System.IO.Directory.GetCurrentDirectory()),
                                    "Error required file", MessageBoxButton.OK, MessageBoxImage.Error);
